...
🤖 **双引擎AI支持**
- **OpenAI引擎**：支持多种输出格式（Function Calling、JSON Object、Structured Output、Text）
- **Google Gemini引擎**：原生结构化输出，高效精准
- **智能切换**：根据配置自动选择最适合的AI提供商
- **API功能测试**：一键检测当前API支持的功能特性
...
- **AI功能**需要额外的API调用费用，但可以显著提高动漫识别准确率
  - OpenAI API：按token计费，gpt-4o-mini性价比高
