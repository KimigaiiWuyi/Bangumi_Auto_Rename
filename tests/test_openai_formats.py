...
def test_output_format_configuration():
    ...
    for fmt in formats:
        ...
        # 验证配置
        assert client.output_format == fmt, f"输出格式配置失败: 期望 {fmt}, 实际 {client.output_format}"
        ...
        if fmt == "function_calling":
            ...
            assert "tool_choice" in request_params, "Function calling模式应该包含tool_choice参数"
            print("✅ Function calling参数配置正确")
        elif fmt == "json_object":
            assert "response_format" in request_params, "JSON object模式应该包含response_format参数"
            assert request_params["response_format"]["type"] == "json_object", "JSON object格式配置错误"
            print("✅ JSON object参数配置正确")
        elif fmt == "structured_output":
            assert "response_format" in request_params, "Structured output模式应该包含response_format参数"
            assert request_params["response_format"]["type"] == "json_schema", "Structured output格式配置错误"
            print("✅ Structured output参数配置正确")
        elif fmt == "text":
            # Text模式不应该有特殊参数
            assert "tools" not in request_params, "Text模式不应该包含tools参数"
            assert "response_format" not in request_params, "Text模式不应该包含response_format参数"
            print("✅ Text模式参数配置正确")
